...
import pandas as pd
Juridico_routes = Blueprint('Juridico', __name__)


@Juridico_routes.route("/EduLink/Juridico/administrar_Alumnos/")
@nocache
@requiere_permisos(permisos_requeridos=["update", "view"], departamento_requerido="juridico")
def Juridico():
    ...
    if 'correo' in session:
        ...
        if administrador:     
            ...
            return render_template("Juridico/juridico.html", alumnos=alumnos_con_progreso, periodos=periodos, administrador=admin,mostrar_modal=not acepto_terminos)
        else:
